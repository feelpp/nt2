 ...
 namespace nt2{ namespace ext
 {
                             ...
                             )
   {
      ...
      BOOST_FORCEINLINE result_type operator()(A0 const& a, A1 const& b) const
     {
       ...
       double anrm = magmablas_dlange('I', m, na, dA.data(), m, dE.data() );
       ...
       magmablas_sgemv( 't', m, na, one, dSR.data(), m, dSX.data(), one, 0, dSX.data(), one );

       magma_sgeqrf2_gpu( m, na, dSR.data(), m, tau.data(), &info );

       magmablas_strsm('l','u','t','n',na,nhrs,(float)one,dSR.data(),m,dSX.data(), na);

       magmablas_strsm('l','u','n','n',na,nhrs,(float)one,dSR.data(),m,dSX.data(), na);

       magmablas_slag2d(ldb,nhrs,dSX.data(),ldb,dX.data(),ldb,&info);

       magmablas_dlacpy('A', ldb, nhrs, dB.data(), ldb, dE.data(), ldb);

       magmablas_dgemv( 'n', lda, na, negone, dA.data(), lda, dX.data(), one, one, dE.data(), one );

       for(size_t i = 1; i<=10;++i)
       {
         magmablas_dgemv( 't', lda, na, one, dA.data(), lda, dE.data(), one, 0, temp.data(), one );
